...
abstract class Payments implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\PaymentsTableMap';
    ...
    /**
     * The value for the checknumber field.
     *
     * @var        string
     */
    ...
    /**
     * Initializes internal state of Base\Payments object.
     */
    ...
    /**
     * Get the [checknumber] column value.
     *
     * @return string
     */
    ...
    /**
     * Get the [optionally formatted] temporal [paymentdate] column value.
     *
     *
    ...
    public function getPaymentdate($format = null)
    {
        if ($format === null) {
            ...
            return $this->paymentdate instanceof \DateTimeInterface ? $this->paymentdate->format($format) : null;
        }
    }
     ...
     * @return $this|\Payments The current object (for fluent API support)
     */
    public function setCustomernumber($v)
    {
        ...
        if ($this->customernumber !== $v) {
            ...
            $this->modifiedColumns[PaymentsTableMap::COL_CUSTOMERNUMBER] = true;
        }
    ...
    /**
     * Set the value of [checknumber] column.
     *
    ...
    public function setChecknumber($v)
    {
        ...
        if ($this->checknumber !== $v) {
            $this->checknumber = $v;
            $this->modifiedColumns[PaymentsTableMap::COL_CHECKNUMBER] = true;
        }
    ...
    /**
     * Sets the value of [paymentdate] column to a normalized version of the date/time value specified.
     *
    ...
    public function setPaymentdate($v)
    {
        ...
        if ($this->paymentdate !== null || $dt !== null) {
            if ($this->paymentdate === null || $dt === null || $dt->format("Y-m-d") !== $this->paymentdate->format("Y-m-d")) {
                $this->paymentdate = $dt === null ? null : clone $dt;
                $this->modifiedColumns[PaymentsTableMap::COL_PAYMENTDATE] = true;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : PaymentsTableMap::translateFieldName('Customernumber', TableMap::TYPE_PHPNAME, $indexType)];
            $this->customernumber = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : PaymentsTableMap::translateFieldName('Checknumber', TableMap::TYPE_PHPNAME, $indexType)];
            $this->checknumber = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : PaymentsTableMap::translateFieldName('Paymentdate', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            $this->paymentdate = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : PaymentsTableMap::translateFieldName('Amount', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 4; // 4 = PaymentsTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Payments'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(PaymentsTableMap::COL_CUSTOMERNUMBER)) {
            $modifiedColumns[':p' . $index++]  = 'customerNumber';
        }
        if ($this->isColumnModified(PaymentsTableMap::COL_CHECKNUMBER)) {
            $modifiedColumns[':p' . $index++]  = 'checkNumber';
        }
        if ($this->isColumnModified(PaymentsTableMap::COL_PAYMENTDATE)) {
            $modifiedColumns[':p' . $index++]  = 'paymentDate';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'customerNumber':
                        ...
                        break;
                    case 'checkNumber':
                        $stmt->bindValue($identifier, $this->checknumber, PDO::PARAM_STR);
                        break;
                    case 'paymentDate':
                        $stmt->bindValue($identifier, $this->paymentdate ? $this->paymentdate->format("Y-m-d H:i:s.u") : null, PDO::PARAM_STR);
                        break;
                    case 'amount':
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setChecknumber($arr[$keys[1]]);
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PaymentsTableMap::COL_CUSTOMERNUMBER)) {
            $criteria->add(PaymentsTableMap::COL_CUSTOMERNUMBER, $this->customernumber);
        }
        if ($this->isColumnModified(PaymentsTableMap::COL_CHECKNUMBER)) {
            $criteria->add(PaymentsTableMap::COL_CHECKNUMBER, $this->checknumber);
        }
        if ($this->isColumnModified(PaymentsTableMap::COL_PAYMENTDATE)) {
            $criteria->add(PaymentsTableMap::COL_PAYMENTDATE, $this->paymentdate);
        }
        if ($this->isColumnModified(PaymentsTableMap::COL_AMOUNT)) {
    ...
    public function hashCode()
    {
        ...
        //relation payments_ibfk_1 to table customers
        if ($this->aCustomers && $hash = spl_object_hash($this->aCustomers)) {
            $primaryKeyFKs[] = $hash;
        } else {
    ...
    {
        $pks = array();
        $pks[0] = $this->getCustomernumber();
        $pks[1] = $this->getChecknumber();
    ...
    {
        $this->setCustomernumber($keys[0]);
        $this->setChecknumber($keys[1]);
    }
    ...
    {
        return (null === $this->getCustomernumber()) && (null === $this->getChecknumber());
    }
    ...
    public function getCustomers(ConnectionInterface $con = null)
    {
        if ($this->aCustomers === null && ($this->customernumber != 0)) {
            ...
            /* The following can be used additionally to
                ...
                $this->aCustomers->addPaymentss($this);
             */
        }
    ...
    {
        if (null !== $this->aCustomers) {
            $this->aCustomers->removePayments($this);
        }
